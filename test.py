...
def speedtest():
    ...
    if not args.simple:
        print_('================= TEST SPEED SERVER =====================')
    config = getConfig()

    if not args.simple:
        print_('Menyambungkan ke Jaringan...')
    ...
    if args.server:
        ...
        if not args.simple:
            print_('Mencari Pilihan Server Terdekat...')
        best = getBestServer(servers)

    if not args.simple:
        print_('Menghubungkan ke Server %(sponsor)s (%(name)s) [%(d)0.2f km]: '
               '%(latency)s ms' % best)
    else:
    ...
    if args.share and args.mini:
        ...
        if not resultid or len(resultid) != 1:
            print_('Gagal Melakukan Test Speed')
            sys.exit(1)

        print_('Berhasil Melakukan Test Speed. Hasil Bisa dilihat di : https://www.speedtest.net/result/%s.png' %
               resultid[0])
